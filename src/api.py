...
class User(db.Model):
    ...
    products_members = db.relationship('ProductMember', backref='user')
...
class Cheque(db.Model):
    ...
    products = db.relationship('Product', backref='cheque')
...
class Product(db.Model):
    ...
    product_members = db.relationship('ProductMember', backref='product')
...
class ProductMember(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('member_id', 'product_id'),)
...
@app.route('/join_product', methods=['PUT'])
@token_required
@fields_required(['cheque_id', 'product_id'])
def join_product(json, user):
    product = Product.query.filter_by(id=json['product_id'], cheque_id=json['cheque_id']).first()
    if product is None:
        return jsonify({'msg': f'cheque {json["cheque_id"]} does not have product {json["product_id"]}'})

    member = ProductMember.query.filter_by(member_id=user.id, product_id=product.id).first()
    if member:
        return jsonify({'msg': 'this user have already joined'}), 400

    member = ProductMember(member_id=user.id, product_id=product.id)
    ...
    return jsonify({'msg': 'user have joined the product'})


@app.route('/add_product', methods=['POST'])
@token_required
@fields_required(['cheque_id', 'name', 'price'])
def add_products(json, user):
    product = Product(
        name=json['name'],
        price=json['price'],
        amount=0,
    ...
    return jsonify({'msg': 'product added', 'id': product.id})


@app.route('/get_products', methods=['POST'])
@token_required
@fields_required(['id'])
def get_products(json, user):
    products = cheque_products(json['id'])
    return jsonify({'msg': products})


@app.route('/get_user_products', methods=['POST'])
@token_required
@fields_required(['cheque_id'])
def get_user_products(json, user):
    products = user_products(json['cheque_id'],user)
    return jsonify({'msg': products})


def user_products(cheque_id, user):
    products = []
    for product_member in ProductMember.query.filter_by(member_id=user.id).all():
        prod = Product.query.filter_by(id=product_member.product_id, cheque_id=cheque_id).first()
        if prod:
            products.append({
                "id": prod.id,
                "name": prod.name,
...
def cheque_products(cheque_id):
    products = []
    for product in Product.query.filter_by(cheque_id=cheque_id).all():
        members = []

        for member in product.product_members:
            user = User.query.filter_by(id=member.member_id).first()
            members.append({
                "id": user.id,
                "name": user.name,
